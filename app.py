...
st.set_page_config(page_title="OP-1 Full Workstation", page_icon="🎛️", layout="wide")

st.title("🎛️ Teenage Engineering OP-1 // Full Motion & Voice Workstation")
st.write("Crea música desde cero con librerías, graba tu voz con el micrófono o traduce el movimiento de tus videos a la cinta.")

# Estado global para las subcapas de video
...
# --- PANEL DE ENTRADA (VIDEO Y VOZ) ---
col_inputs, col_synth = st.columns([1, 1.4])

with col_inputs:
    st.subheader("📥 Fuentes de Entrada (Input Options)")
    ...
    with col_mic:
        st.write("Graba un sonido o tarareo directo desde tu micrófono:")
        recorded_audio = st.audio_input("Graba tu voz:")
        if recorded_audio:
            st.audio(recorded_audio)
            st.info("💡 Tu voz se encuentra lista en el buffer para ser usada como Sampler o Pista de usuario en el OP-1.")

with col_synth:
    st.subheader("🎹 OP-1 Workstation")
    ...
    <head>
      ...
      <style>
        ...
        /* BARRA DE MODOS PRINCIPALES (4 MAIN MODES) */
        .main-modes { display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; margin-bottom: 10px; }
        .btn-mode {
        ...
        .btn-mode.active { background: #ff0055; color: white; border-color: #d30043; }

        /* PÁGINAS T1-T4 SUB-MODULES */
        ...
        .btn-t.active { background: #00ffcc; color: #000; border-color: #00cca3; }
    ...
    <body>
      ...
      <div class="op1-chassis">
        ...
        <div class="main-modes">
          <button id="mSynth" class="btn-mode active">🎹 SYNTH</button>
          <button id="mDrum" class="btn-mode">🥁 DRUM</button>
          <button id="mTAPE" class="btn-mode">📼 TAPE</button>
          <button id="mMixer" class="btn-mode">🎚️ MIXER</button>
        </div>

        <!-- T1 - T4 SUB-MODULES -->
        <div class="t-pages">
          <button id="t1" class="btn-t active">T1: ENGINE</button>
          <button id="t2" class="btn-t">T2: ENVELOPE</button>
          <button id="t3" class="btn-t">T3: EFFECT</button>
          <button id="t4" class="btn-t">T4: MASTER</button>
        </div>

        <!-- OLED DISPLAY -->
        <div class="op1-screen">
          <div class="screen-top">
            <span>STATUS: <b id="lblStatus">STOPPED</b></span>
        ...
        <!-- HERRAMIENTAS DE EDICIÓN DE CINTA -->
        <div style="font-size: 8px; font-weight: bold; color: #444; margin-bottom: 3px;">EDICIÓN DE CINTA (TAPE EDITING):</div>
        <div class="tape-edit-grid">
          ...
          <button id="btnClearTrack" class="btn-act">🗑️ CLEAR</button>
        </div>
      ...
      <script>
        ...
        // GRABACIÓN MASTER Y EXPORTACIÓN WAV
        ...
        // FUNCIONES DE EDICIÓN DE CINTA (LIFT, DROP, SPLIT)
